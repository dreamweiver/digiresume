  ...
  return (
    <Document>
      <Page size="A4" style={styles.page}>
        ...
        {data.about ? <><Text style={styles.sectionTitle}>About</Text><Text style={styles.entryDescription}>{data.about}</Text></> : null}
        {data.skills.length > 0 ? (
          <><Text style={styles.sectionTitle}>Skills</Text>
          <View style={styles.skillsRow}>{data.skills.map((s) => <Text key={s} style={styles.skill}>{s}</Text>)}</View></>
        ) : null}
